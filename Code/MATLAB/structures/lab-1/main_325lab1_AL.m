...
% Import aluminum specimin data
dataFileAL = importdata(buildPath('AEE325\Lab-1\data\AL_cdl.txt'));
[~, uniqueIdx] = unique(dataFileAL.data(:,2), 'stable');
cleanData = dataFileAL.data(uniqueIdx,:);
...
validIdx = sigma_t > 0.09*max(sigma_t);
...
elasticIdx_eng = find(nSlope_eng > 0.1E11, 1, "last");
...
elasticIdx_t = find(nSlope_t > 0.1E11, 1, "last");
...
logRange_min = find(epsilon_t(epsilon_t-epsilony_t < 0.0001), 1, "last");
logRange_max = find(epsilon_t == eu_t);
...
title('Engineering Stress-Strain Diagram', '2024 Aluminum')
...
legend('Eng. stress-strain curve', ...
    ...
    'Fracture strength', ...
    'Location','best');
xlim([0 0.104]); ylim([0 0.26]);
hold off;

q = findobj('type','figure');
% autosave(q, 'Eng_SS_AL', 'AEE325\Lab-1\figs');

% True diagram
figure(22); clf; hold on; grid on;
title('True Stress-Strain Diagram', '2024 Aluminum')
xlabel('Strain, e_i (m/m)');
ylabel('Stress, S_i (GPa)');
...
legend('True stress-strain curve', ...
    ...
    'Fracture strength', ...
    'Location','best');
xlim([0 0.1]); ylim([0 .26]);
hold off;

q = findobj('type','figure');
% autosave(q, 'True_SS_AL', 'AEE325\Lab-1\figs');

% Log plot
figure(23); clf; hold on; grid on;
title('True Stress-Strain Log-Log Plot', '2024 Aluminum')
xlabel('Strain, e_i (m/m)');
ylabel('Stress, S_i (GPa)');
...
ylim([log_S_t(1), log_S_t(end)]);
hold off;

q = findobj('type','figure');
% autosave(q, 'LL_AL', 'AEE325\Lab-1\figs');
